...
def ptspr_on():
    ...
    row = p.shape[0]
    col = p.shape[1]
    topic = open("data/user-topic-distro.txt", 'r')
    PTSPR = []
    off_vec=ptspr_off()
    for line in topic:
        data = line.split(' ')
        ...
        PTSPR.append(online_q.sum(axis=0))
    print("{} secs for pts_pagerank_online".format(round(time.time() - ptspr_on_start,4)),"/ round(time,4)")
    return PTSPR
